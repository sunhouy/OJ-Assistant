...
class User {
    ...
    // 用户注册
    public function register($username, $password, $machineCode, $inviteCode = null) {
        // 检查用户名是否已存在
        ...
        try {
            ...
            // 插入用户数据，包含邀请人信息
            $sql = "INSERT INTO users (username, password, machine_code, inviter) VALUES (?, ?, ?, ?)";
            $result = $this->db->execute($sql, [$username, $hashedPassword, $machineCode, $inviteCode]);
            if (!$result['success']) {
            ...
            // 如果有邀请码，检查该机器码是否已经被注册过
            if ($inviteCode) {
                // 检查该机器码是否已经存在（除了当前正在注册的用户）
                $sql = "SELECT COUNT(*) as count FROM users WHERE machine_code = ? AND username != ?";
                $result = $this->db->query($sql, [$machineCode, $username]);
                $row = $result->fetch_assoc();
                
                // 只有当该机器码是第一次注册时，才奖励会员
                if ($row['count'] == 0) {
                    // 给新注册用户送1天会员
    ...
    // 用户登录
    public function login($username, $password, $machineCode) {
        // 先查询是否存在该用户名
    ...
    // 查询会员状态
    public function checkMemberStatus($username, $machineCode) {
        $sql = "SELECT is_member, expire_date, created_at, last_login FROM users WHERE username = ? AND machine_code = ?";
        $result = $this->db->query($sql, [$username, $machineCode]);
