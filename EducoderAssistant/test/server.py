...
class ChatServer:
    def __init__(self, host='0.0.0.0', ws_port=8765, http_port=8080):
        ...
        self.paired_clients: Dict[str, str] = {}  # python_client_id -> web_client_id
    ...
    async def server_status_api(self, request):
        ...
        return web.json_response({
            ...
            'active_tokens': len([t for t in self.otp_tokens.values() if not t.used]),
            'paired_sessions': len(self.paired_clients)
        })
    ...
    async def handle_websocket(self, websocket: WebSocketServerProtocol, path: str):
        ...
        try:
            ...
            # Web 客户端连接
                ...
                self.paired_clients[python_client_id] = client_id
            ...
            async for message in client.websocket:
                try:
                    ...
                    if message_type == 'message':
                        # 转发消息给配对的Web客户端
                        web_client_id = self.paired_clients.get(client.client_id)
                        if web_client_id:
                            ...
                            if web_client:
                                ...
                                logger.info(f"转发消息从Python到Web: {data.get('text')}")
            ...
            async for message in web_client.websocket:
                try:
                    ...
                    if message_type == 'message':
                        ...
                        logger.info(f"转发消息从Web到Python: {data.get('text')}")
    ...
    async def cleanup_client(self, client_id: str):
        ...
        if client:
            # 如果是Python客户端，断开配对的Web客户端
            if client.client_type == 'python':
                web_client_id = self.paired_clients.get(client_id)
                if web_client_id:
                    web_client = self.clients.get(web_client_id)
            ...
            # 如果是Web客户端，找到配对的Python客户端
            elif client.client_type == 'web':
                for py_id, web_id in list(self.paired_clients.items()):
                    if web_id == client_id:
