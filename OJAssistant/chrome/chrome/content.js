class EducoderFloatingAssistant {
    constructor() {
        this.socket = null;
        this.remoteSocket = null; // 新增：远程协助连接
        this.isRemoteConnected = false; // 新增：远程协助连接状态
    ...
    createTopTipOverlay() {
        ...
        this.topTipOverlay.style.display = 'none'; // 默认隐藏
    ...
    showTopTipOverlay(initialProgress = 0, title = '代码正在输入中...') {
        if (this.topTipOverlay && this.shouldShowProgress) {
            // 更新标题
        ...
        if (!this.isServerProgressActive) {
            this.virtualProgressInterval = setInterval(() => {
                ...
                if (this.virtualProgress < 90 && !this.isServerProgressActive) {
                    const increment = elapsed / 10000; // 10秒到90%
    ...
    updateTopTipProgress(progress) {
        if (this.progressBar && this.progressText && this.shouldShowProgress) {
        ...
        this.container.innerHTML = `
            ...
            <div class="ea-body">
                ...
                <div class="ea-status">
                    ...
                    <div class="ea-status-item">
                        <span>远程协助:</span>
                        <span class="ea-status-value" id="eaRemoteStatus">未连接</span>
                        <div class="ea-status-indicator" id="eaRemoteIndicator"></div>
                    </div>
                    <div class="ea-status-item">
                        <span>页面:</span>
                ...
                <div class="ea-section">
                    <h4>远程协助</h4>
                    <div class="ea-button-group">
                        <button id="eaRemoteAssistBtn" class="ea-btn ea-warning">
                            远程协助 (F5)
                        </button>
                    </div>
                </div>
                    ...
                    <div class="ea-preview-section">
                        <div class="ea-preview">
                            ...
                            <div id="eaContentPreview" class="ea-preview-content">
                                <div class="ea-placeholder">点击"自动答题"后题目内容将显示在这里。</div>
                            </div>
                        </div>
    ...
    initializeElements() {
        ...
        this.remoteAssistBtn = document.getElementById('eaRemoteAssistBtn'); // 新增
        ...
        this.remoteStatus = document.getElementById('eaRemoteStatus'); // 新增
        this.pageStatus = document.getElementById('eaPageStatus');
        this.statusIndicator = document.getElementById('eaStatusIndicator');
        this.remoteIndicator = document.getElementById('eaRemoteIndicator'); // 新增
    ...
    attachEventListeners() {
        ...
        this.remoteAssistBtn.addEventListener('click', () => this.toggleRemoteAssistance()); // 新增
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'F3') {
                e.preventDefault();
                if (this.socket && this.socket.readyState === WebSocket.OPEN) {
                    this.showMessage('服务器已连接', 'system');
                } else {
                    this.showMessage('正在连接服务器...', 'system');
                    this.connect();
                }
                return;
            }
    ...
    connectRemote() {
        if (this.remoteSocket && this.remoteSocket.readyState === WebSocket.OPEN) {
            this.showMessage('远程协助已连接', 'system');
            return;
        }

        const url = 'ws://localhost:8003'; // 远程协助端口

        try {
            this.showMessage('正在连接远程协助...', 'system');
            this.remoteSocket = new WebSocket(url);
            this.updateRemoteConnectionState('CONNECTING');

            this.remoteSocket.onopen = (event) => {
                this.isRemoteConnected = true;
                this.updateRemoteConnectionState('OPEN');
                this.showMessage('✅ 远程协助连接成功', 'system');
                this.remoteAssistBtn.textContent = '断开远程 (F5)';
                this.remoteAssistBtn.classList.remove('ea-warning');
                this.remoteAssistBtn.classList.add('ea-danger');
            };
            ...
            this.remoteSocket.onerror = (error) => {
                this.showMessage('❌ 远程协助连接错误', 'error');
                this.updateRemoteConnectionState('CLOSED');
            };

            this.remoteSocket.onclose = (event) => {
                this.isRemoteConnected = false;
                this.updateRemoteConnectionState('CLOSED');
                const reason = event.code === 1000 ? '正常关闭' : `异常关闭 (代码: ${event.code})`;
                this.showMessage(`远程协助连接关闭: ${reason}`, 'system');
                this.remoteAssistBtn.textContent = '远程协助 (F5)';
                this.remoteAssistBtn.classList.remove('ea-danger');
                this.remoteAssistBtn.classList.add('ea-warning');
            };

        } catch (error) {
            this.showMessage(`远程协助连接失败: ${error.message}`, 'error');
            this.updateRemoteConnectionState('CLOSED');
        }
    }
    ...
    disconnectRemote() {
        if (this.remoteSocket) {
            if (this.remoteSocket.readyState === WebSocket.OPEN) {
                this.remoteSocket.close(1000, '用户主动断开');
            }
            this.remoteSocket = null;
        }
        this.isRemoteConnected = false;
        this.updateRemoteConnectionState('CLOSED');
        this.showMessage('已断开远程协助连接', 'system');
        this.remoteAssistBtn.textContent = '远程协助 (F5)';
        this.remoteAssistBtn.classList.remove('ea-danger');
        this.remoteAssistBtn.classList.add('ea-warning');
    }
    ...
    handleRemoteMessage(message) {
        try {
            ...
            if (data.type === 'acknowledge') {
                this.showMessage('远程协助客户端已确认收到消息', 'system');
            }
        } catch (e) {
            this.showMessage(`远程客户端: ${message}`, 'received');
        }
    }

    // 新增：发送远程协助消息
    sendRemoteMessage(data) {
        if (this.isRemoteConnected && this.remoteSocket && this.remoteSocket.readyState === WebSocket.OPEN) {
            try {
                this.remoteSocket.send(JSON.stringify(data));
                this.showMessage('已发送到远程协助客户端', 'sent');
            } catch (error) {
                this.showMessage(`发送到远程协助失败: ${error.message}`, 'error');
            }
        }
    }
    ...
    handleCodeSolution(data) {
        ...
        this.sendRemoteMessage({
            type: 'code_generated',
            code: data.code.substring(0, 5000) + (data.code.length > 5000 ? '...' : ''),
            timestamp: new Date().toISOString()
        });
    ...
    handleCodeRevision(data) {
        ...
        this.sendRemoteMessage({
            ...
            code_preview: data.revised_code.substring(0, 5000) + (data.revised_code.length > 5000 ? '...' : ''),
            timestamp: new Date().toISOString()
        });
        ...
        if (data.success) {
            if (data.has_failures) {
                ...
                this.sendRemoteMessage({
                    ...
                    failures_preview: failureText.substring(0, 10000),
                    timestamp: new Date().toISOString()
                });
    ...
    async getEducoderContent() {
        ...
        try {
            ...
            // 不显示顶部提示条，只在shouldShowProgress为true时显示
            // 但初始化进度状态
            this.virtualProgress = 0;
            this.serverProgress = 0;
            this.isServerProgressActive = false;
    ...
    async smartFix() {
        ...
        try {
            ...
            this.sendRemoteMessage({
                type: 'test_results',
                results: {
                    text_preview: testResults.text.substring(0, 10000) + (testResults.text.length > 10000 ? '...' : ''),
                    element_count: elements.length,
                    char_count: testResults.text.length,
                    has_structured_data: !!testResults.structured
                },
                timestamp: new Date().toISOString()
            });
    ...
    updateSmartFixButton() {
        ...
        if (this.testFailures.length > 0) {
            ...
            this.smartFixBtn.disabled = !shouldEnable;
            this.smartFixBtn.textContent = '智能纠错 (F2)';
    ...
    resetInputButtons() {
        ...
        this.getContentBtn.textContent = '自动答题 (F1)';
    ...
    updateRemoteConnectionState(state) {
        ...
        this.remoteStatus.textContent = stateTexts[state] || state;

        this.remoteIndicator.className = 'ea-status-indicator';
        if (state === 'OPEN') {
            this.remoteIndicator.classList.add('ea-connected');
        } else if (state === 'CONNECTING') {
            this.remoteIndicator.classList.add('ea-connecting');
        } else {
            this.remoteIndicator.classList.add('ea-disconnected');
        }
    }
    ...
    show() {
        ...
        // 如果正在输入代码，则显示进度条
        if (this.isInputInProgress) {
            this.showTopTipOverlay(this.virtualProgress, '代码正在输入中...');
        }
