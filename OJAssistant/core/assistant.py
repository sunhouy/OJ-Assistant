...
class EducoderAssistant:
    def __init__(self, gui, model_info=None):
        ...
        # 使用传入的模型信息，如果未传入则使用默认值
        if model_info:
            self.model_name = model_info.get('model', 'qwen3-coder-plus')
            self.base_url = model_info.get('base_url', 'https://dashscope.aliyuncs.com/compatible-mode/v1')
            self.api_key = model_info.get('api_key', '')
        else:
            # 如果没有传入模型信息，使用默认值（向后兼容）
    ...
    async def handle_educoder_content_auto_input(self, websocket, data):
        ...
        try:
            ...
            self.gui.log(f"当前使用模型: {self.model_name}")
            ...
            try:
                # 检查是否使用远程服务器
                use_remote = self.gui.config_manager.get_setting('remote_assist_enabled', 'False',
                                                                 'REMOTE_ASSIST').lower() == 'true'

                if use_remote:
                    # 使用远程服务器，需要通过HTTP API发送
                    remote_url = self.gui.config_manager.get_setting('remote_assist_server_url', 'ws://localhost:8001',
                                                                     'REMOTE_ASSIST')
                    # 将ws://转换为http://用于HTTP请求
                    http_url = remote_url.replace('ws://', 'http://').replace('wss://', 'https://')
                    # 移除路径部分，只保留协议和域名
                    if '/ws' in http_url:
                        http_url = http_url.split('/ws')[0]
                    device_id = self.gui.machine_code or self.gui.config_manager.get_machine_code()

                    # 发送HTTP请求到远程服务器
                    import requests
                    try:
                        response = requests.post(
                            f"{http_url}/api/send_question",
                            json={
                                'device_id': device_id,
                        ...
                        if response.status_code == 200:
                            ...
                            if result.get('success'):
                                self.gui.log(f"题目内容已发送到远程服务器")
                            else:
                                self.gui.log(f"远程服务器返回错误: {result.get('message')}")
                        else:
                            self.gui.log(f"发送题目到远程服务器失败: HTTP {response.status_code}")
                    except requests.exceptions.RequestException as e:
                        self.gui.log(f"发送题目到远程服务器失败: {e}")
                elif hasattr(self.gui, 'remote_assist_server') and self.gui.remote_assist_server:
                    # 使用本地服务器
                    device_id = self.gui.machine_code or self.gui.config_manager.get_machine_code()
                    if device_id and self.gui.remote_assist_server.is_device_connected(device_id):
                        self.gui.remote_assist_server.send_question_to_device(device_id, question_content)
            except Exception as e:
                self.gui.log(f"发送题目到远程协助服务器失败: {e}")
